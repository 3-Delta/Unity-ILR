...
public class CLRRedirectionDemo : MonoBehaviour
{
...
#if UNITY_ANDROID
        WWW www = new WWW(ILRSettings.HotfixDllFullPath);
#else
        WWW www = new WWW("file:///" + ILRSettings.HotfixDllFullPath);
#endif
        while (!www.isDone)
            yield return null;
...
#if UNITY_ANDROID
        www = new WWW(ILRSettings.HotfixPdbFullPath);
#else
        www = new WWW("file:///" + ILRSettings.HotfixPdbFullPath);
#endif
        while (!www.isDone)
            yield return null;
        if (!string.IsNullOrEmpty(www.error))
    ...
    unsafe void OnHotFixLoaded()
    {
        ...
        appdomain.Invoke("HotFix.TestCLRRedirection", "RunTest", null, null);
    }
